...
| `test/ledger.test.mjs`, `test/rt2_crash.test.mjs`, `test/rt2_recovery.test.mjs` | in the CI suite |

## Storage
`node:sqlite` (`--experimental-sqlite`). Conservative pragmas: `journal_mode=WAL`,
...
Uniqueness: **`UNIQUE(network, canon, claim_day)`** (the entitlement), `UNIQUE(client_idempotency_key)`.
Separate `reservations(network, txid, vout PRIMARY KEY, claim_id)` — a UTXO can be reserved by
at most one claim, durably, committed **atomically** with the claim.
...
## UTXO reservation
`pickFaucetCoins` excludes outpoints reserved by any non-terminal claim (`activeReservations`),
then `createAuthorised` inserts the claim **and** its reservation rows in one transaction; a
...
invent past claims). First run creates `schema_version=1`.
